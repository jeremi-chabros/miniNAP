...
function filterTrace(traces, fs, lowpass, highpass)
    ...
    filter_order = 3
    responsetype = Bandpass(lowpass, highpass; fs)
...
function plotHeatmap(values_original, ref_el_id, stim_elec_id, annotate_flag, marker_size, cbar_title, min_val, max_val, type)
    ...
    for (ctr, channel) in enumerate(chan_labels)
        ...
        if type == "diff"
            ...
            color_id = Int64.(round.(rescale(values[.~isnan.(values)], 1, 256)))
            color_value = colormap[color_id[ctr]]
            cgrads = :viridis
            hM[1, 1] = max_val
            hM[8, 1] = min_val
        end
...
function main(filename)
    ...
    lowpass = 200
    highpass = 4_000
    threshold_multiplier = 5.0 # times median absolute deviation of the signal
    duration_s = size(traces, 1) / fs
    
    # Filtering (not sure if needs to be done)
    # filtered_traces = filterTrace(traces, fs, lowpass, highpass)
    ...
    num_active_channels = sum(spk_freq .> 0.1)
    active_channel_ids = findall(spk_freq .> 0.1)
